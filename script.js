...
let greetingMessage = `Hello, ${userName}! Here is a list of users in our database.`;
document.querySelector('h1').innerText = greetingMessage;
...
class UserDirectoryManager {
    ...
    loadData() {
        ...
        if (storedUserData) {
            return new Promise((resolve) => resolve(storedUserData))
                .then(users => {
                    this.users = users;
                    return users;
                });
